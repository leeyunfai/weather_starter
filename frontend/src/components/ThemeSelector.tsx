    ...
    <div ref={containerRef} className="relative">
      <button
        ...
        onKeyDown={(e) => {
          ...
          if (e.key === 'Enter' && !isOpen) { e.preventDefault(); setIsOpen(true); }
        }}
        ...
        >
          {themes.map((id) => (
            <li
              ...
              tabIndex={0}
              onClick={() => { setTheme(id as any); setIsOpen(false); }}
              onKeyDown={(e) => { if (e.key === 'Enter') { setTheme(id as any); setIsOpen(false); } if (e.key === 'Escape') setIsOpen(false); }}
